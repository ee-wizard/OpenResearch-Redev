...
export type BadgeVariant = "default" | "success" | "error" | "warning" | "primary";
...
export function Badge({ variant = "default", size = "default", className, ...props }: HTMLAttributes<HTMLSpanElement> & { variant?: BadgeVariant; size?: "default" | "small" }) {
  return (
    <span
      className={cn(
        "badge inline-flex items-center rounded-full border py-px font-sans shadow-control-subtle",
        size === "small" ? "px-1.5 text-xs font-normal" : "px-2 text-sm font-medium",
        VARIANTS[variant],
        className,
      )}
      {...props}
